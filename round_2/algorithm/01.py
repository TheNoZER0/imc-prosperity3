"""
Zac's r1 submission, but ive implemented basic trading for the new instruments.
"""
...
class Strategy:
    ...
    def _place_orders(state: Status, fair_price, delta_b, delta_a, order_amount):
    ...
    def mm_glft(state: Status, fair_price, mu, sigma, gamma=1e-9, order_amount=50):
        ...
        return Strategy._place_orders(state, fair_price, delta_b, delta_a, order_amount)

    
    def mm_ou(state: Status, fair_price, gamma=1e-9, order_amount=0):
        q = state.rt_position / order_amount
        ...
        vfucn = lambda q, Q: -INF if (q == Q + 1 or q == -(Q + 1)) else \
            math.log(math.sin(((q + Q + 1) * math.pi) / (2 * Q + 2)))

        delta_b = 1 / gamma * math.log(1 + gamma / kappa_b) - \
                  1 / kappa_b * (vfucn(q + 1, Q) - vfucn(q, Q))
        delta_a = 1 / gamma * math.log(1 + gamma / kappa_a) + \
                  1 / kappa_a * (vfucn(q, Q) - vfucn(q - 1, Q))
...
class Trade:
    ...
    def croissant(state: Status) -> list[Order]:
        current_price = state.maxamt_midprc
        orders = []
        orders.extend(Strategy.arb(state=state, fair_price=current_price))
        return orders
    ...
    def jam(state: Status) -> list[Order]:
        current_price = state.maxamt_midprc
        orders = []
        orders.extend(Strategy.arb(state=state, fair_price=current_price))
        return orders

    def djembe(state: Status) -> list[Order]:
        current_price = state.maxamt_midprc
        orders = []
        orders.extend(Strategy.arb(state=state, fair_price=current_price))
        return orders

    def picnic_basket1(state: Status) -> list[Order]:
        current_price = state.maxamt_midprc
        orders = []
        orders.extend(Strategy.arb(state=state, fair_price=current_price))
        return orders

    def picnic_basket2(state: Status) -> list[Order]:
        current_price = state.maxamt_midprc
        orders = []
        orders.extend(Strategy.arb(state=state, fair_price=current_price))
        return orders
...
class Trader:
    ...
    state_croissant = Status("CROISSANTS")
    state_jam= Status("JAM")
    state_djembe= Status("DJEMBE")
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        result["CROISSANT"]=Trade.croissant(self.state_croissant)
        result["JAM"]=Trade.jam(self.state_jam)
        result["DJEMBE"]=Trade.djembe(self.state_djembe)
        result["PICNIC_BASKET1"]=Trade.picnic_basket1(self.state_picnic_basket1)
        result["PICNIC_BASKET2"]=Trade.picnic_basket2(self.state_picnic_basket2)
