...
def parse_algorithm(algorithm: Path) -> Any:
    ...
    sys.path.append(str(algorithm.parent))  # Append the directory to sys.path
    return import_module(algorithm.stem)  # Import the algorithm module
...
def backtest(
    ...
    merge_pnl: bool = True,
    use_visualiser: bool = False,
    output_log_file: Optional[str] = None,
    skip_output: bool = False,
    print_stdout: bool = False,
    no_trades_matching: bool = False,
    ...
    args = Namespace(
        algorithm=algorithm_file_path,
        data=data_path,
        days=days if days is not None else rounds,
        merge_pnl=merge_pnl,
        vis=use_visualiser,
        out=output_log_file,
        no_out=skip_output,
        print=print_stdout,
        no_trades_matching=no_trades_matching,
        original_timestamps=original_timestamps,
        no_progress=no_progress,
        vis_requests=vis_requests,
    )
    ...
    parsed_days = parse_days(file_reader, args.days)
    ...
    # Calculate and return total profit
    total_profit = 0
    for result in results:
        last_timestamp = result.activity_logs[-1].timestamp
        for row in reversed(result.activity_logs):
