...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        result = {}
        conversions = 0
        trader_data = "Unified Strategy: Round2 Adaptive + Improved Round1"
        ...
        if len(hist) >= EMA_PERIOD:
            ...
            if len(hist) >= EMA_PERIOD + 1:
                ...
                derivative = 0
            logger.print("Round2 Diagnostics:", "EMA =", ema_val, "std =", std, "current =", current, "z =", z, "derivative =", derivative)
            if abs(z) < Z_THRESHOLD or derivative * (prev_z if 'prev_z' in locals() else 1) < 0:
                use_ema_strategy = True

        # For Round 2, always use arbitrage (i.e. trade everything).
        ...
        for asset in ["RAINFOREST_RESIN", "KELP", "SQUID_INK"]:
            ...
            if asset == "RAINFOREST_RESIN":
                ...
                orders_r1 = Trade_R1.squink(status)  # This uses our improved conservative SQUID_INK strategy.
            else:
                orders_r1 = []
